...
const ServiceCategoryComponent = () => {
  ...
  const enableServiceCategoryData = () => {
    if (!isEmpty(checkedValues)) {
      ...
      dispatch(serviceCategory.actions.enableServiceCategory(formData))
    } else {
  ...
  const disableServiceCategoryData = () => {
    if (!isEmpty(checkedValues)) {
      ...
      dispatch(serviceCategory.actions.disableServiceCategory(formData))
    } else {
      toast.error('No data Selected')
    }
  }
  ...
  const handleToggleAction = (data: {[key: string]: string}) => {
    data?.status
      ? dispatch(serviceCategory.actions.singleDisableServiceCategory({id: data.id}))
      : dispatch(serviceCategory.actions.singleEnableServiceCategory({id: data.id}))
  }
  ...
  useEffect(() => {
    ...
    if (enableSuccess) {
      toast.success('Service category enabled successfully')
    }
    if (disableSuccess) {
      toast.success('Service category disabled successfully')
    }
    handleChecked([])
    dispatch(serviceCategory?.actions.getServiceCategory(params))
  ...
  const serviceCategoryData = data?.grievanceServiceCategory
    ? data?.grievanceServiceCategory?.map((item: any, i: number) => ({
        ...
        name: item.name,
        nameNp: item.nameNp,
        showAccountNumberField: item.showAccountNumberField,
  ...
  const handleDelete = () => {
    let serviceCategoryId = checkedValues?.map((value) => ({id: value}))
    dispatch(serviceCategory?.actions?.deleteServiceCategory(serviceCategoryId))
    handleAlertClose()
  }
  const handleRefresh = () => {
    dispatch(serviceCategory.actions.getServiceCategory(params))
