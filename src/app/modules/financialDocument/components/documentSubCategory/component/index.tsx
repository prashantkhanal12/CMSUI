...
import AddDocumentSubCategory from './AddDocumentSubCategory'
import {toast} from 'react-toastify'
import {isEmpty} from 'lodash'
import DeleteModal from 'src/app/modules/common/components/deleteModal'
import DesignComponent from 'src/app/modules/common/components/designComponent'

const DocumentSubCategoryComponent = () => {
  ...
  const enableDocumentSubCategoryData = () => {
    if (!isEmpty(checkedValues)) {
      const formData = checkedValues.map((d: string) => ({
        id: d,
      }))
      dispatch(documentSubCategory.actions.enableDocumentSubCategory(formData))
    } else {
      toast.error('No data Selected')
    }
  }

  const disableDocumentSubCategoryData = () => {
    if (!isEmpty(checkedValues)) {
      const formData = checkedValues.map((d: string) => ({
        id: d,
      }))
      dispatch(documentSubCategory.actions.disableDocumentSubCategory(formData))
    } else {
      toast.error('No data Selected')
    }
  }
  ...
  const handleToggleAction = (data: {[key: string]: string}) => {
    data?.status
      ? dispatch(documentSubCategory.actions.singleDisableDocumentSubCategory({id: data.id}))
      : dispatch(documentSubCategory.actions.singleEnableDocumentSubCategory({id: data.id}))
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      flexGrow: 1,
      cell: <Cell>{(rowData) => rowData.documentCategory.name}</Cell>,
      sortable: false,
    },
    {
      label: 'Slug',
  ...
  const documentSubCategoryData = data?.documentSubCategory
    ? data?.documentSubCategory?.map((item: any, i: number) => ({
  ...
  const handleDelete = () => {
    let documentSubCategoryId = checkedValues?.map((value) => ({id: value}))
    dispatch(documentSubCategory?.actions?.deleteDocumentSubCategory(documentSubCategoryId))
    handleAlertClose()
  }
  ...
  const handleSubmitSort = (data: Array<ItemType>) => {
    let body = {
      documentSubCategoryId: data?.map((item) => ({id: item?.keyId})),
    }
    dispatch(documentSubCategory.actions.sortDocumentSubCategory(body))
  }

  useEffect(() => {
    if (!isEmpty(sortDocumentSubCategoryData) && success) {
      toast.success('Member sub type sorted successfully')
      dispatch(documentSubCategory.actions.sortDocumentSubCategoryReset())
      setParams({...params, limit: 10})
      dispatch(documentSubCategory.actions.getDocumentSubCategory(params))
      setOpenSortModal(false)
    }
  }, [sortDocumentSubCategoryData, success])

  return (
    <>
      <div className='shadow p-3 bg-white rounded'>
        <DesignComponent
          moduleName='Document Sub Category'
          params={params}
          setParams={setParams}
          ...
          exportShow={false}
          importShow={false}
          enableMultiple={enableDocumentSubCategoryData}
          disableMultiple={disableDocumentSubCategoryData}
          data={documentSubCategoryData}
          sortShow={true}
          sortButtonName='Sort'
...
export default DocumentSubCategoryComponent
