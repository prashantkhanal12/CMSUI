...
const AddPosition = ({open, params, handleClose, actionType, editSelectedData}: Props) => {
  const dispatch = useDispatch()
  const [thumbImage, setThumbImage] = useState('')
  const [coverImage, setCoverImage] = useState('')
  const [linkData, setLinkData] = useState('')
  const {loading, success} = useSelector((state: any) => state.position)
  ...
  useEffect(() => {
    if (success) {
      ...
      dispatch(positionRedux?.actions.resetPosition())
      handleClose()
    }
  }, [success])
  const statusOptions = [
  ...
  return (
    <div className='modal-container'>
      ...
      >
        <Modal.Header>
          <Modal.Title>{actionType} Position</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div>
            <Formik
              ...
              onSubmit={(values, {setSubmitting}) => {
                ...
                if (!isEmpty(editSelectedData)) {
                  dispatch(positionRedux.actions.updatePosition(formData, editSelectedData?.id))
                } else {
                  dispatch(positionRedux.actions.addPosition(formData))
                }
              }}
            >
